describe('Basic user flow for Website', () => {
  ...
  it('Make sure <product-item> elements are populated', async () => {
    ...
    for (let i = 0; i < prodItems.length; i++) {
      let rawData = await prodItems[i].getProperty('data');
      let jsonValue = await rawData.jsonValue();
      if (jsonValue.title.length == 0) { allArePopulated = false; }
      if (jsonValue.price.length == 0) { allArePopulated = false; }
      if (jsonValue.image.length == 0) { allArePopulated = false; }
    }
    expect(allArePopulated).toBe(true);
  }, 10000);
  ...
  it('Clicking the "Add to Cart" button should change button text', async () => {
    ...
    if (jsonValue !== 'Remove from Cart') { swap = false; }
  ...
  it('Checking number of items in cart on screen', async () => {
    ...
    let countText = await cartCount.getProperty('innerText');
    let countValue = await countText.jsonValue();
  ...
  it('Checking number of items in cart on screen after reload', async () => {
    ...
    for (let i = 0; i < prodItems.length; i++) {
      ...
      let innerText = await button.getProperty('innerText');
      let jsonValue = await innerText.jsonValue();
      if (jsonValue !== 'Remove from Cart') { reload = false; }
    }
  ...
  it('Checking number of items in cart on screen after reload', async () => {
    ...
    for (let i = 0; i < prodItems.length; i++) {
      ...
      let innerText = await button.getProperty('innerText');
      let jsonValue = await innerText.jsonValue();
      if (jsonValue !== 'Add to Cart') { empty = false; }
    }
